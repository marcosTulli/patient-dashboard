...
const useForm = ({ formFields }: { formFields: FormFields }) => {
  ...
  const keys = Object.keys(formFields).reduce<Record<string, string>>(
    (acc, key) => {
      acc[key] = key;
      return acc;
    },
  ...
  const formInitialValues = Object.entries(formFields).reduce<
  ...
  const validationSchema = Yup.object(
    Object.entries(formFields).reduce<Record<string, Yup.AnySchema>>(
      (acc, [key, field]) => {
        if (field?.validation) {
